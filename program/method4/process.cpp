...
void Process::Compute(Process in)
{
	...
	Point_3 temp_;
	c1_[0].x = -1;//-0.0003;//0.007;
	c1_[0].y = 0.0001;//0.8766;//0.4813;
	c1_[0].z = 0.0008;//-0.4813;//0.8766;
	c2_[0].x = 0.5510;//0.7962;//-0.2501;
	c2_[0].y = -0.7079;//0.6046;//0.3651;
	c2_[0].z = 0.4418;//-0.7079;//0.8968;
	...
	for(unsigned long i=0;i<3;++i)
		...
		{
			M = RotateMet(c1_[i],c2_[j]);
			char fname[30];
    sprintf(fname,"./after/%d%d.off",i,j);
